    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-black via-blue-950 to-purple-950 text-white px-4 py-10">
            ...
            <div className="max-w-7xl mx-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-12">
                {[1, 2, 3].map((i) => (
                    <Card key={i} className="bg-black/40 border-white/10 p-6 flex items-center gap-5 overflow-hidden relative">
            ...
            <div className="max-w-7xl mx-auto grid grid-cols-1 sm:grid-cols-3 gap-5 mb-12">
                {[1, 2, 3].map((i) => (
                    <Card key={i} className="bg-black/40 border-white/10 p-5 flex flex-col items-center gap-3 overflow-hidden relative">
            ...
            <div className="max-w-7xl mx-auto">
                <Skeleton className="h-8 w-40 bg-white/10 mb-5" />
                <Card className="bg-black/40 border-white/10 overflow-hidden relative">
                    <div className="p-4 space-y-4">
                        <div className="flex border-b border-white/10 pb-2">
                            {[1, 2, 3, 4].map((h) => <Skeleton key={h} className="h-4 flex-1 bg-white/10 mx-2" />)}
                        </div>
                        {[1, 2, 3, 4, 5].map((r) => (
                            <div key={r} className="flex gap-4">
                                {[1, 2, 3, 4].map((c) => <Skeleton key={c} className="h-6 flex-1 bg-white/5" />)}
                            </div>
                        ))}
                    </div>
                    <div className="absolute inset-0 -translate-x-full bg-gradient-to-r from-transparent via-white/5 to-transparent animate-shimmer" />
